...
class Game extends Model
{
    ...
    public function scopeSingleplayerLeaderboard($query, $type)
    {
        ...
        return $query
            ->select(
                ...
                DB::raw('SUM(CASE WHEN (winner_user_id = player1_user_id AND player1_points BETWEEN 91 AND 119) OR (winner_user_id = player2_user_id AND player2_points BETWEEN 91 AND 119) THEN 1 ELSE 0 END) AS total_capotes'),
                // BANDEIRA = winner gets exactly 120 points
                DB::raw('SUM(CASE WHEN (winner_user_id = player1_user_id AND player1_points = 120) OR (winner_user_id = player2_user_id AND player2_points = 120) THEN 1 ELSE 0 END) AS total_bandeiras'),
            ...
            ->whereNull('match_id') // ONLY games vs bot
    ...
    public function scopeMultiplayerLeaderboard($query, $type)
    {
        ...
        return $query
            ...
            ->where('games.type', $type)
            ->whereNotNull('winner_user_id')
            ->whereNull('match_id') // EXCLUDE bot games
